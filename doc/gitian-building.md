...
*Setup instructions for a gitian build of Volix using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Volix
...
Clone the git repositories for volix and gitian and then checkout the volix version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/volix-crypto/volix.git
cd volix
...
Follow the instructions in [doc/release-process.md](release-process.md) in the volix repository
...
To build Volix (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the volix repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/volix/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/volix-crypto/volix
...
For example:
```bash
URL=https://github.com/crowning-/volix.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit volix=${COMMIT} --url volix=${URL} ../Volix/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit volix=${COMMIT} --url volix=${URL} ../Volix/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit volix=${COMMIT} --url volix=${URL} ../Volix/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/volix-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/volix-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/volix-build.assert
```
...
[volix/gitian.sigs](https://github.com/volix-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
