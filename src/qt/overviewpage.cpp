...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::VOX)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeVolixAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVolixAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeVolixAmount = strAnonymizeVolixAmount.remove(strAnonymizeVolixAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVolixAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeVolixAmount * COIN) nMaxToAnonymize = nAnonymizeVolixAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeVolixAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeVolixAmount));
        strAnonymizeVolixAmount = strAnonymizeVolixAmount.remove(strAnonymizeVolixAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVolixAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
