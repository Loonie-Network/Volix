...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VolixMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("VolixMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("VolixMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("volix-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running VolixMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
